...
const initializeDBAndServer = async () => {
  try {
    ...
    app.listen(3000, () => {
      console.log('Server is running of https://localhost:3000/')
    })
  } catch (error) {
    console.log('DataBae error is ${error.message}')
    process.exit(1)
  }
}
...
app.get('/todos/', async (request, response) => {
  ...
  switch (true) {
    ...
    case hasPriorityProperty(request.query):
      if (priority === 'HIGH' || priority === 'MEDIUM' || priority === 'LOW') {
        getTodosQuery = `
            SELECT * FROM todo WHERE category = 'priority = '${priority}';`
        data = await database.all(getTodosQuery)
        response.send(data.map(eachItem => outPutResult(eachItem)))
      } else {
        response.status(400)
...
app.put('/todos/:todosId/', async (request, response) => {
  ...
  const previousTodoQuery = `SELCT * FROM todo WHERE id = ${todoId};`
